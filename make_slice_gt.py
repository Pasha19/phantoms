...
from make_phantom import resampled_volume
...
def create_gt_sliced_thin(ex: xc.CatSim, d: float, thin_scale: float = 0.05) -> np.ndarray:
    ...
    voxel_size = d / ex.cfg.scanner.detectorColCount
    for i in range(phantom_cfg["n_materials"]):
        ...
        size = y_size * phantom_cfg["cols"][i]
        new_size_in_vox = int(size / voxel_size + 0.5)
        if new_size_in_vox != volume.shape[1]:
            volume, y_size = resampled_volume(volume, y_size, z_size, new_size_in_vox)
...
def main() -> None:
    ...
    vol_gt = create_gt_sliced_thin(xcist, d=501.7847226)

    tifffile.imwrite(phantom_path.parent / "phantom.gt.tif", vol_gt, compression="zlib")
    # vol_gt = (vol_gt - vol_gt.min()) / (vol_gt.max() - vol_gt.min())
    # im = Image.fromarray(255 * vol_gt[vol_gt.shape[0] // 2, :, :]).convert("L")
    # im.save(phantom_path.parent / "phantom.gt.png")
