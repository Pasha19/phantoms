...
def Detection_OpticalCrosstalk_FlatPanel(viewIn, cfg):
    ...
    viewOut_size_2 = int(cfg.scanner.detectorRowCount * cfg.physics.FlatPanel_OSfactor)
    viewOut = viewIn.reshape(viewOut_size_2, viewOut_size)
    
    
#   Model the optical crosstalk as a Lorentzian function in frequency domain
    lorentz_row_temp = np.arange(2*viewOut_size_2+1, dtype=np.float32) + 1.0
    lorentz_row_temp = lorentz_row_temp - (viewOut_size_2+1)
...
#   Perform convolution in frequency domain
    viewOut_freq = np.fft.fft2(viewOut, s=(2*viewOut_size_2+1, 2*viewOut_size+1))
    ...
    viewOut_final = viewOut_filt_spatial[0:viewOut_size_2, 0:viewOut_size]
